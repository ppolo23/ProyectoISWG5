...
class Asignatura(Model):
    ...
    """
    def __init__(self, nombre: str=None, codigo: str=None, tipo: str=None, num_creditos: int=None, curso: str=None):
        """
    ...
    def curso(self, curso: str):
        ...
        :type curso: str
        """
        allowed_values = ["1º", "2º", "3º", "4º"]
        if curso not in allowed_values:
            raise ValueError(
                "Invalid value for `curso` ({0}), must be one of {1}"
                .format(curso, allowed_values)
            )
